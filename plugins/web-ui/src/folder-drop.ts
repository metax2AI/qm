...
import { formatByteCount, formatNumber } from "./localization.ts";
    ...
    for (const entry of await readAllEntries(dir)) {
      ...
      if (out.length >= caps.maxFiles) {
        throw new FolderDropError(
          msg(
            str`"${root.name}" has too many files (over ${formatNumber(caps.maxFiles)}) — zip it yourself or drop a subfolder.`,
          ),
        );
      }
      ...
      if (bytes > caps.maxBytes) {
        throw new FolderDropError(
          msg(
            str`"${root.name}" is too big (over ${formatByteCount(caps.maxBytes)}) — zip it yourself or drop a subfolder.`,
          ),
        );
      }
...
export async function folderToZipFile(root: DropEntryLike, caps: FolderCaps = DEFAULT_CAPS): Promise<File> {
  ...
  if (!files.length) throw new FolderDropError(msg(str`"${root.name}" has no files in it.`));
