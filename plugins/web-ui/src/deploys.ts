...
function statusLabel(d: DeploymentView): string {
  ...
  if (!d.status) return msg("Unknown");
  if (d.status === "running") return msg("Running");
  if (d.status === "stopped") return msg("Stopped");
  if (d.status === "archived") return msg("Archived");
  return d.status;
}
...
function permissionBadge(d: DeploymentView): TemplateResult {
  ...
  const title = manage
    ? msg("You own this app or have permission to manage it.")
    : msg("This app is shared with a context you can access. You can open and clone it, but not change it.");
  return html`<span class="deploy-permission ${manage ? "manage" : "view"}" title=${title}
    >${manage ? msg("Can manage") : msg("Can view")}</span
  >`;
}

function versionLabel(d: DeploymentView): string {
  if (d.currentVersion === undefined) return msg("Version unknown");
  if (d.appliedVersion !== undefined && d.appliedVersion !== d.currentVersion)
    return msg(str`v${d.appliedVersion} live · v${d.currentVersion} pending`);
  return `v${d.currentVersion}`;
}

function deployedLabel(d: DeploymentView): string {
  const at = deploymentLatestAt(d);
  return at ? msg(str`Deployed ${relTime(at)}`) : msg("Deployment time unavailable");
}

function ownerLabel(d: DeploymentView): string {
  const me = appState.me?.user;
  if (d.ownerScopeId === `personal:${me}`) return msg("Your personal context");
  if (d.ownerScopeId?.startsWith("personal:"))
    return msg(str`${friendlyPrincipal(d.ownerScopeId.slice("personal:".length))} · Personal`);
  if (d.ownerScopeId?.startsWith("org:")) return msg("Organization");
  return d.createdBy ? msg(str`Shared context · created by ${friendlyPrincipal(d.createdBy)}`) : msg("Shared context");
}
...
function deployTabs(): TemplateResult {
  ...
  return html`
    <div class="cron-list-controls" role="tablist" aria-label=${msg("App view")}>
      ${tabs.map(
        (tab) => html`
          <button
            type="button"
          ...
          >
            <span>${tab.label}</span><span class="cron-filter-count">${formatNumber(counts[tab.value])}</span>
          </button>
        `,
      )}
    </div>
...
function deploymentRow(d: DeploymentView): TemplateResult {
  ...
  return html`
    <div class="list-row deploy-row ${d.status === "archived" ? "deploy-row-archived" : ""}">
      ...
      <div class="deploy-row-actions" aria-label=${msg("App actions")}>
        ${
          running && d.webUrl
            ? html`<a class="btn deploy-open" href=${withBase(d.webUrl)} target="_blank" rel="noreferrer"
                >${msg("Open")} ${icon(ExternalLink, 14)}</a
              >`
            : nothing
        }
        ${
          d.webUrl
            ? html`<button
                ...
                type="button"
                title=${msg("Copy app URL")}
                aria-label=${msg("Copy app URL")}
  ...
  return html`
    <div class="session-menu deploy-menu">
      <button
        ...
        type="button"
        aria-label=${msg(str`More actions for ${deploymentTitle(d)}`)}
        aria-haspopup="menu"
              ...
              ${
                d.status === "archived"
                  ? html`<button
                    ...
                    >
                      ${icon(RotateCcw, 15)}<span>${msg("Restore")}</span>
                    </button>`
                  : html`
                      <button
                        class="session-menu-option"
                      ...
                      >
                        ${icon(Pencil, 15)}<span>${msg("Edit display name")}</span>
                      </button>
                      <button
                        class="session-menu-option"
                        type="button"
                      ...
                      >
                        ${icon(Pencil, 15)}<span>${msg("Change URL slug")}</span>
                      </button>
                      <button
...
function drawDeploysPage(): void {
  ...
  else if (deployLoading && deployList.length === 0) empty = msg("Loading apps…");
  else if (deployQuery && allForTab.length) empty = msg("No apps match your search.");
  else if (deployScope) empty = msg("No apps in this context.");
  ...
  render(
    html`
      ${listPageTpl({
        ...
        action: { label: msg("Deploy with Agent"), onClick: deployWithAgent },
        controls: html`<label class="deploy-sort"
          ><span>${msg("Sort")}</span>${fieldSelect({
            compact: true,
            ariaLabel: msg("Sort apps"),
            value: deploySort,
            onChange: (value) => {
            ...
            options: [
              html`<option value="newest">${msg("Newest")}</option>`,
              html`<option value="name">${msg("Name")}</option>`,
              html`<option value="status">${msg("Status")}</option>`,
            ],
          })}</label
        >`,
        search: {
          value: deployQuery,
          placeholder: msg("Search apps"),
          onInput: (value) => {
            deployQuery = value;
            drawDeploysPage();
          },
...
async function openDeploy(d: DeploymentView): Promise<void> {
  ...
  try {
    ...
    deployNotices = withDeploymentDetailNotice(
      deployNotices,
      d.id,
      errMessage(error, msg("Could not load app details.")),
    );
    drawDeployDetail(d);
  }
}
  ...
  render(
    html`
      <div class="resource-detail deploy-detail">
        ${listBackLink(msg("Apps"), returnToDeploysList)}
        <div class="resource-heading deploy-detail-heading">
          ...
          <div class="actions">
            ${running && d.webUrl ? html`<a class="btn primary" href=${withBase(d.webUrl)} target="_blank" rel="noreferrer">${msg("Open app")} ${icon(ExternalLink, 14)}</a>` : nothing}
            ${running && canManage(d) ? html`<button class="btn" type="button" @click=${(event: Event) => void openLiveEdit(d, event.currentTarget as HTMLButtonElement)}>${icon(Pencil, 14)}<span>${msg("Edit live")}</span></button>` : nothing}
            ${d.webUrl ? html`<button class="btn" type="button" @click=${(event: Event) => void copyText(new URL(withBase(d.webUrl!), window.location.href).href, event.currentTarget as HTMLButtonElement)}>${icon(Copy, 14)}<span>${msg("Copy URL")}</span></button>` : nothing}
          </div>
        </div>
        ${loading ? html`<div class="hint">${msg("Loading authoritative app details…")}</div>` : nothing}
        ...
        <section class="deploy-detail-section">
          ...
          <div class="deploy-facts">
            <div><span>${msg("Status")}</span><strong>${statusLabel(d)}</strong></div>
            <div>
              <span>${msg("Live version")}</span><strong>${d.appliedVersion ?? d.currentVersion ?? "—"}</strong>
            </div>
            <div><span>${msg("Latest version")}</span><strong>${d.currentVersion ?? "—"}</strong></div>
            <div>
              <span>${msg("Last deployed")}</span
              ><strong
                >${deploymentLatestAt(d) ? formatDateTime(deploymentLatestAt(d), { dateStyle: "medium", timeStyle: "short" }) : "—"}</strong
              >
            </div>
            <div>
              <span>${msg("Last opened")}</span
              ><strong>${d.lastAccessAt ? relTime(d.lastAccessAt) : msg("No recorded access")}</strong>
            </div>
            <div><span>${msg("Access")}</span><strong>${permissionBadge(d)}</strong></div>
          </div>
        </section>

        <section class="deploy-detail-section">
          <h3>${msg("Ownership and access")}</h3>
          <div class="field">
            <label>${msg("Created in")}</label>
            <div class="value">${contextScope ? scopeChip(contextScope) : msg("Unknown")}</div>
          </div>
          <div class="field">
          ...
          ${
            d.gitUrl
              ? html`<div class="field deploy-git-field">
                  <label>${msg("Git remote")}</label>
                  <div class="value">
                    <code>${d.gitUrl}</code
                    ><button
                      class="icon-btn subtle"
                      type="button"
                      title=${msg("Copy Git remote")}
                      aria-label=${msg("Copy Git remote")}
                  ...
                  <p class="hint">
                    ${
                      d.permission === "write"
                        ? msg("Clone or push a new version with this short-lived authenticated URL.")
                        : msg("Clone source with this short-lived read-only authenticated URL.")
                    }
                  </p>
                </div>`
              : nothing
          ...
          canManage(d)
            ? html`<section class="deploy-detail-section">
                ...
                <div class="deploy-setting-row">
                  <div>
                    <strong>${msg("Display name")}</strong
                    ><span>${msg("The human-friendly name shown here. This does not change the app URL.")}</span>
                  </div>
                  ${editingName ? deployEditForm(d, "displayName") : html`<div class="deploy-setting-value"><span>${d.displayName || msg("Using URL slug")}</span><button class="btn" type="button" @click=${() => startEditDeploy(d, "displayName")}>${msg("Edit")}</button></div>`}
                </div>
                <div class="deploy-setting-row">
                  <div>
                    <strong>${msg("URL slug")}</strong
                    ><span>${msg("Changes the app URL. Existing links do not redirect.")}</span>
                  </div>
                  ${editingSlug ? deployEditForm(d, "name") : html`<div class="deploy-setting-value"><code>/d/${deploymentSlug(d)}/</code><button class="btn" type="button" @click=${() => startEditDeploy(d, "name")}>${msg("Change")}</button></div>`}
                </div>
                <div class="actions deploy-danger-actions">
                  ${
                    d.status === "archived"
                      ? html`<button class="btn" type="button" @click=${() => void restoreDeploy(d)}>
                          ${icon(RotateCcw, 14)}<span>${msg("Restore deployment")}</span>
                        </button>`
                      : html`<button
                          ...
                          @click=${() => requestArchive(d)}
                        >
                          ${icon(Archive, 14)}<span>${msg("Archive deployment")}</span>
                        </button>`
                  }
                </div>
              </section>`
        ...
        <section class="deploy-detail-section">
          ...
          ${
            versions.length
                    ...
                    (version) => html`
                      <div class="deploy-version-row">
                        <div>
                          ...
                          >${version.version === d.appliedVersion ? html`<span class="badge ok">${msg("Live")}</span>` : nothing}${version.version === d.currentVersion && version.version !== d.appliedVersion ? html`<span class="badge">${msg("Latest")}</span>` : nothing}
                        </div>
                        <div>
                          <span>${formatDateTime(version.createdAt, { dateStyle: "medium", timeStyle: "short" })}</span
              ...
              : html`<div class="empty compact">${msg("No version history available.")}</div>`
          }
        </section>
      </div>
      ${archiveCandidate ? archiveDialog(archiveCandidate) : nothing} ${deployToast ? undoToast(deployToast) : nothing}
...
function deployEditForm(d: DeploymentView, field: "displayName" | "name"): TemplateResult {
  ...
  return html`
    ...
    >
      <label>
        <span class="deploy-slug-input ${slug ? "" : "name"}"
          >${slug ? html`<span>/d/</span>` : nothing}<input
            ...
            aria-label=${slug ? msg("URL slug") : msg("Display name")}
      ...
      <button class="icon-btn" type="submit" title=${msg("Save")} aria-label=${msg("Save")} ?disabled=${deploySaving}>
        ${icon(Check, 14)}
      </button>
      <button
        class="icon-btn"
...
async function commitEditDeploy(d: DeploymentView): Promise<void> {
  ...
  if (field === "name" && !value) {
    deployNotices = withDeploymentDetailNotice(deployNotices, d.id, msg("A URL slug is required."));
  ...
  try {
    ...
    if (currentDeployActionView(d.id) === "target") {
      ...
      deployToast = { deployment: updated, text: msg(str`${deploymentTitle(updated)} settings saved.`) };
      drawCurrentDeployView();
    }
  } catch (error) {
    deploySaving = false;
    const message = errMessage(error, msg("Could not save app settings."));
    if (currentDeployActionView(d.id) === "target") {
    ...
    >
      ...
      >
        <div class="project-dialog-head">
          <div><h2 id="deploy-archive-title">${msg(str`Archive ${deploymentTitle(d)}?`)}</h2></div>
        </div>
        <p>
          ${msg(
        ...
        <div class="project-dialog-actions actions">
          <button class="btn" type="button" data-dialog-cancel @click=${closeArchiveDialog}>${msg("Cancel")}</button>
          <button
...
async function archiveDeploy(d: DeploymentView): Promise<void> {
  ...
  if (activeDeploy?.id === d.id)
    deployNotices = withDeploymentDetailNotice(deployNotices, d.id, msg("Archiving deployment…"));
  else deployNotices = withDeploymentListNotice(deployNotices, msg("Archiving deployment…"));
  setDeployBackgroundInert(false);
  archiveCandidate = null;
  drawCurrentDeployView();
  try {
    ...
    deployToast = {
      ...
      text: msg(str`${deploymentTitle(d)} is offline and archived.`),
      undo: true,
    };
    await refreshDeployments();
    ...
    deploySaving = false;
    const message = errMessage(error, msg("Could not archive deployment."));
    if (currentDeployActionView(d.id) === "target") {
      deployNotices = withDeploymentDetailNotice(deployNotices, d.id, message);
      drawDeployDetail(activeDeploy!);
    } else {
...
async function restoreDeploy(d: DeploymentView): Promise<void> {
  ...
  deploySaving = true;
  deployMenuId = null;
  if (restoringActive) deployNotices = withDeploymentDetailNotice(deployNotices, d.id, msg("Restoring deployment…"));
  else if (!activeDeploy) deployNotices = withDeploymentListNotice(deployNotices, msg("Restoring deployment…"));
  ...
  try {
    ...
    deployToast = { deployment: restoredResponse, text: msg(str`${deploymentTitle(d)} is restored and running.`) };
    ...
    deploySaving = false;
    const message = errMessage(error, msg("Could not restore deployment."));
    if (currentDeployActionView(d.id) === "target") {
      deployNotices = withDeploymentDetailNotice(deployNotices, d.id, message);
      drawDeployDetail(activeDeploy!);
    } else {
...
function undoToast(toast: { deployment: DeploymentView; text: string; undo?: boolean }): TemplateResult {
  ...
  return html`<div class="deploy-toast" role="status">
    ...
    >${archived ? html`<button type="button" ?disabled=${deploySaving} @click=${() => void restoreDeploy(toast.deployment)}>${msg("Undo")}</button>` : nothing}<button
      class="icon-btn"
      type="button"
      title=${msg("Dismiss")}
      aria-label=${msg("Dismiss notification")}
      @click=${() => {
        deployToast = null;
        drawCurrentDeployView();
      }}
...
async function openLiveEdit(d: DeploymentView, button: HTMLButtonElement): Promise<void> {
  ...
  try {
    ...
    if (!r.url) throw new Error(msg("no live URL for this app"));
    ...
    deployNotices = withDeploymentDetailNotice(
      deployNotices,
      d.id,
      errMessage(error, msg("Could not open live editing.")),
    );
...
function deployWithAgent(): void {
  ...
  conv.newChat();
  conv.composer.state.draft = `${msg("Deploy an app for me.")} `;
...
async function refreshDeployments(): Promise<"updated" | "failed" | "superseded"> {
  ...
  try {
    ...
    if (seq !== deployRefreshSeq) return "superseded";
    deployNotices = withDeploymentListNotice(deployNotices, errMessage(error, msg("Failed to load apps.")));
    return "failed";
  } finally {
