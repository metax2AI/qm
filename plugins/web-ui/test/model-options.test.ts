...
test("runtime options render and select native DeepSeek models", () => {
  applyRuntimeOptions(
    null,
    ["pi"],
    { pi: ["deepseek-v4-flash", "deepseek-v4-pro"] },
    { harnessId: "pi", modelId: "deepseek-v4-flash" },
  );
  assert.deepEqual(
    getModelOptions().map((option) => [option.model.id, option.model.provider]),
    [
