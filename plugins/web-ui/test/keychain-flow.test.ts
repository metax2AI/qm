...
test("destructive controls settle duplicate attempts while a mutation is busy", () => {
  ...
  assert.match(connectorsSource, /connectorNotice = msg\("Another keychain change is still in progress\."\)/);
...
test("keychain rows reserve success badges for actionable states", () => {
  ...
  assert.match(connectorsSource, /expired \? html`<span class="kc-state warning">\$\{msg\("Expired"\)\}<\/span>` : ""/);
  assert.match(connectorsSource, /<span class="kc-state warning">\$\{msg\("Reconnect needed"\)\}<\/span>/);
});
