...
test("mobile sidebar is modal, dismissible, and sized for touch", () => {
  ...
  assert.match(
    shell,
    /class="sidebar-scrim"[^>]+aria-label=\$\{msg\("Close sidebar"\)\}[^>]+@click=\$\{toggleSidebar\}/,
  );
...
test("touch layouts expose row actions and preserve readable composer choices", () => {
  ...
  assert.match(contexts, /project-create-button"\s+type="button"\s+aria-label=\$\{msg\("New project"\)\}/);
  assert.match(
    compactCss,
