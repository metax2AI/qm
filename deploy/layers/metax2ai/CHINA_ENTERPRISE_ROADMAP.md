...
- 用户页的「Users」整卡英文，以及所有计数标签（审计页显示「200 events」）。计数由
  `plural()` 拼接英文名词加 `s` 得到，改为按语言分支后中文侧输出「200 条事件」。
...
- **英文界面截图未取得。** M1.5 要求「中英文桌面端关键路径截图」，本轮只走了中文；
  语言切换后刷新是否保留选择也未验证。
...
- **XFS 项目配额代码从未在真实 XFS 主机上执行过。** `src/sandbox/xfs-project-quota.ts` 的 preflight、项目 ID 分配与 collision 防护只有单元测试，新增的探测只覆盖 rootfs 配额，不覆盖 home 配额。
