...
- 中国版 Web 服务的无配置运行时默认值和根、插件环境模板均已设置为 `zh-Hans`；M4 选定部署目标并生成正式 layer 时，仍把同一非秘密环境值写入 `env.web-ui`。
- browser-only 生产形态实例已使用本地 PostgreSQL 启动并通过服务健康检查；中英文桌面端与移动端截图仍待 Firefox 自动化权限开放后完成。
