...
- 状态：执行中（M3 Runner 已实现并完成真机隔离与出网验收，三项待补：真实重启恢复、XFS 主机上的 home 配额、独立评审）
- 更新日期：2026-08-10
...
| 超时、资源耗尽、异常容器可回收并审计  | 停泊误判已修；超时策略待决策（见下） |
...
1. 完成 M3 的独立评审并合入。三个 PR 分别是 Runner 本体、出网默认拒绝、依赖安全
   升级；出网那个改的是所有沙箱后端共用的策略代码，需单独评审，且单独合并会立即
   让现有部署的沙箱默认断网。
