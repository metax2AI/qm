...
- 状态：执行中（M1 开发完成、在线验收待凭据，M1.5 待启动）
- 更新日期：2026-08-05
...
- `codex/china-deepseek-provider` 已在提交 `882efc5` 中完成原生 provider、Pi 运行时、模型目录、凭据、Admin、Web UI、CLI、部署配置和 Browse 子任务接入。
- 自动化验证、类型检查、lint、格式检查和独立代码审查已经完成。
- M1 的开发已结束，但真实 DeepSeek Key 与 Slack 开发工作区下的在线端到端验收仍待具备环境后执行；该环境验收必须在 M2 演示验收前完成，不再扩大 M1 的开发范围。
...
- `main` 只保存已经验证的稳定能力。
- 每个里程碑或高风险能力使用独立短分支，不建立长期漂移的“中国版”分支。
- 通用功能保持客户无关，便于提交上游；客户名称、地址、凭据和业务配置只进入 `deploy/layers/<org>/`。
- 同步上游使用 merge，不使用 rebase 改写私有分支历史。
...
2. 提交 M1 的 DeepSeek 原生接入，并在具备真实凭据后补充在线端到端验收。
3. 从最新稳定 `upstream/main` 创建 `codex/web-ui-localization`，将通用实现独立提交上游；私有层只设置中国版默认语言。
